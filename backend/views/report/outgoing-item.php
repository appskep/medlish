...
<div class="detail-view-container" style="padding:<?= isset($to_pdf) && $to_pdf ? '0px' : '20px' ?>">
    <div class="printable">
        ...
        foreach ($models as $model) {
            ...
            $subtotal_modal = $model->item->purchase_net_price * $model->quantity;
        ...
        ?>
            <tr>
                ...
                <?php if (!$salesman_id) { ?><td><?= $model->outgoing->salesman->name ?></td><?php } ?>
                <?php if (!$customer_id) { ?><td><?= $model->outgoing->customer->name ?></td><?php } ?>
                ...
                <?php if ($with_profit) { ?><td class="text-right"><?= Yii::$app->formatter->asDecimal($model->item->purchase_net_price, 0) ?></td><?php } ?>
                <td class="text-right"><?= Yii::$app->formatter->asDecimal($model->price, 0) ?></td>
                <?php if ($with_profit) { ?><td class="text-right"><?= Yii::$app->formatter->asDecimal($profit, 0) ?></td><?php } ?>
                ...
                <?php if ($with_profit) { ?><td class="text-right"><?= Yii::$app->formatter->asDecimal($subtotal_profit, 0) ?></td><?php } ?>
            </tr>
        <?php } ?>
        </table>
